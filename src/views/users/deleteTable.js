...
const DeletedUserTableData = () => {
  ...
  if(res.data.msg =='201'){
   ...
   const allDeletedUsers = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/deletedUser_details`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
  ...
  if(res.data.msg =='201'){
    ...
    setDeletedUserData(res.data.feedAll)
...
const paginationFunction = async() =>{
  try {
    const res = await client.get(`/api/deletedUser_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
      headers: {
      'Authorization': 'Bearer '+userTokenId,
      }
    })
  ...
  return (
    <Card>
      ...
      {loadingData &&
        ...
        {!loadingData && deletedUserData.length > 0 ?
      ...
      {!loadingData && deletedUserData.length < 1 &&
        <NoRecordFund />
        }
      </TableContainer>
      <ShowSnackbar
...
export default DeletedUserTableData
