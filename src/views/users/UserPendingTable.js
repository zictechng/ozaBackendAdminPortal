...
const UserPendingTableData = () => {
  ...
  if(res.data.msg =='201'){
   ...
   const allPendingUsers = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/pendingUser_details`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
  ...
  if(res.data.msg =='201'){
    ...
    setPendingUserData(res.data.feedAll)
  ...
  const paginationFunction = async() =>{
    try {
      const res = await client.get(`/api/pendingUser_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
  ...
  return (
    <Card>
      ...
      <TableContainer>
        ...
        {!loadingData && pendingUserData.length > 0 ?
            ...
            {pendingUserData.map(row => (
              <TableRow hover key={row._id} sx={{ '&:last-of-type td, &:last-of-type th': { border: 0 } }}>
                ...
                <Stack direction="row" spacing={1}>
                  <Link href={`/users/view-user/${row._id}`} passHref>
                  <Chip
                    label={'View'}
                    color={'primary'}
                    sx={{
      ...
      {!loadingData && pendingUserData.length < 1 &&
        <NoRecordFund />
        }
      </TableContainer>
...
export default UserPendingTableData
