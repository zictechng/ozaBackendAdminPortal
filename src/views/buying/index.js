...
const [searchInput, setSearchInput] = useState({
  searchValue: '',
   ...
   // get current user transaction stats here
    ...
    setLoadingData(true);
    try {
      const res = await client.get(`/api/userBuyOrder_details`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
  ...
  if(res.data.msg =='201'){
    ...
    setAllBuyData(res.data.feedAll)
  ...
  const paginationFunction = async() =>{
    try {
      const res = await client.get(`/api/userBuyOrder_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
...
allBuyOrders()
  ...
  return (
    <Card>
      ...
      <CardHeader title='Buy Service Transactions' titleTypographyProps={{ variant: 'h6' }} />
        <Stack direction={'row'} >
      ...
      {loadingData &&
        ...
        {!loadingData && allBuyData.length > 0 ?
      ...
      {!loadingData && allBuyData.length < 1 &&
        <NoRecordFund />
        }
      </TableContainer>
        ...
        {loading ?
          ...
          <Box>
            ...
            <Typography gutterBottom sx={{fontSize: '0.975rem !important', textAlign:'justify' }}>
              ...
              Transaction Date: <strong>{fetchData.creditOn !== null ? moment(fetchData.creditOn).format('YYYY-MM-DD'): ''} </strong><br />
              Action Date: <strong>{fetchData.approved_date !== null ? moment(fetchData.approved_date).format('YYYY-MM-DD'): ''}</strong> <br /><br/>
