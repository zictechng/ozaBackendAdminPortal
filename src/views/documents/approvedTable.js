...
const UserApprovedTable = () => {
  const router = useRouter()

  const [approvedDocumentUserData, setApprovedDocumentUserData] = useState([]);
  ...
  if(res.data.msg =='201'){
   ...
   const allApprovedDocument = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/approvedDocument_details`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
  ...
  if(res.data.msg =='201'){
    ...
    setApprovedDocumentUserData(res.data.feedAll)
 ...
 const paginationFunction = async() =>{
  try {
    const res = await client.get(`/api/approvedDocument_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
      headers: {
      'Authorization': 'Bearer '+userTokenId,
      }
    })
    ...
    <Card>
      <Stack direction={'row'} justifyContent={'space-between'} spacing={1}>
        ...
        <Stack direction={'row'} >
          ...
          {approvedDocumentUserData.length > 0 &&
          <Box sx={{
           // marginTop: 10,
      ...
      <TableContainer>
        ...
        {!loadingData && approvedDocumentUserData.length > 0 ?
        <Table sx={{ minWidth: 800 }} aria-label='table in dashboard'>
          ...
          <TableBody>
            {approvedDocumentUserData.map(row => (
              <TableRow hover key={row._id} sx={{ '&:last-of-type td, &:last-of-type th': { border: 0 } }}>
                ...
                <TableCell>{moment(row.action_date).format('YYYY-MM-DD')}</TableCell>
      ...
      {!loadingData && approvedDocumentUserData.length < 1 &&
        <NoRecordFund />
        }
      </TableContainer>
        <ShowSnackbar
...
export default UserApprovedTable
