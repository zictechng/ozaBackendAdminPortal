...
const UserDocumentRejectedTable = () => {
  const router = useRouter()

  const [rejectedDocumentUserData, setRejectedDocumentUserData] = useState([]);
  ...
  if(res.data.msg =='201'){
   ...
   const allRejectedDocument = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/rejectedDocument_details`, {
        headers: {
        'Authorization': 'Bearer '+userTokenId,
        }
      })
 ...
 const paginationFunction = async() =>{
  try {
    const res = await client.get(`/api/rejectedDocument_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
      headers: {
      'Authorization': 'Bearer '+userTokenId,
      }
    })
    ...
    <Card>
      ...
      <CardHeader title='User Documents' titleTypographyProps={{ variant: 'h6' }} />
        <Stack direction={'row'} >
          ...
          {rejectedDocumentUserData.length > 0 &&
          <Box sx={{
            //marginTop: 10,
      ...
      {loadingData &&
        ...
        {!loadingData && rejectedDocumentUserData.length > 0 ?
        <Table sx={{ minWidth: 800 }} aria-label='table in dashboard'>
          ...
          <TableBody>
            {rejectedDocumentUserData.map(row => (
      ...
      {!loadingData && rejectedDocumentUserData.length < 1 &&
        <NoRecordFund />
        }
      </TableContainer>
        <ShowSnackbar
...
export default UserDocumentRejectedTable
