...
const AdminDeletedUserView = () => {
  ...
  const userTokenId = localStorage.getItem('userToken')
   ...
   const allActiveUsers = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/adminDeletedUser_details`, {
        headers: {
...
const paginationFunction = async() =>{
  try {
    const res = await client.get(`/api/adminDeletedUser_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
      headers: {
      'Authorization': 'Bearer '+userTokenId,
      }
    })
...
export default AdminDeletedUserView
