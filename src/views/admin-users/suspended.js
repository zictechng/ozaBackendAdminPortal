...
const AdminSuspendedUserView = () => {
  ...
  const userTokenId = localStorage.getItem('userToken')
   ...
   const allActiveUsers = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/adminSuspendedUser_details`, {
        headers: {
...
const paginationFunction = async() =>{
  try {
    const res = await client.get(`/api/adminSuspendedUser_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
      headers: {
      'Authorization': 'Bearer '+userTokenId,
      }
    })
            ...
            {activeUserData.map(row => (
              <TableRow hover key={row._id} sx={{ '&:last-of-type td, &:last-of-type th': { border: 0 } }}>
                ...
                <Stack direction="row" spacing={1}>
                    ...
                    <Link
                  href={`/admin-user/view-user/${row._id}`} passHref>
                  <Chip
                    label={'View'}
                    color={'primary'}
                    sx={{
...
export default AdminSuspendedUserView
