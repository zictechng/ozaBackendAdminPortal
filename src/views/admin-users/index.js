...
const AdminActiveUserView = () => {
  ...
  const userTokenId = localStorage.getItem('userToken')
   ...
   const allActiveUsers = async() =>{
    setLoadingData(true);
    try {
      const res = await client.get(`/api/adminActiveUser_details`, {
        headers: {
...
const paginationFunction = async() =>{
  try {
    const res = await client.get(`/api/adminActiveUser_details?pageNumber=${pageNumber}&pageLimit=${pageLimit}`, {
      headers: {
      'Authorization': 'Bearer '+userTokenId,
      }
    })
...
export default AdminActiveUserView
